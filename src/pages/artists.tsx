    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        ...
        <p className="text-gray-500 dark:text-gray-400 mt-1">
          Discover top makeup artists near you
        </p>
      </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {artists.map((artist: any) => (
              ...
              >
                ...
                >
                  <Heart
                    className={`w-4 h-4 ${isFavorite(String(artist.id)) ? "fill-white" : ""}`}
                  />
                </button>
                <Link to={`/artists/${artist.id}`}>
                  <div className="p-5">
                    <div className="flex flex-wrap gap-1.5 mb-3">
                      {(artist.categories || [])
                        ...
                        .map((cat: any, ci: number) => (
                          <span
                            key={ci}
                            className="px-2.5 py-0.5 text-[11px] font-medium bg-rose-50 dark:bg-rose-950/50 text-rose-600 dark:text-rose-400 rounded-full"
                          >
                            {cat.name}
                          </span>
                        ))}
