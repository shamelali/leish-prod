          ...
          <div className="relative rounded-2xl overflow-hidden">
            ...
            <div className="absolute bottom-6 left-6 right-6">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <div className="flex items-center gap-4 mt-2 text-white/70 text-sm">
                    ...
                    <span className="flex items-center gap-1">
                      ...
                      {artist.response_time || "Fast response"}
                    </span>
                  </div>
                </div>
                <button
                ...
                >
                  <Heart
                    className={`w-5 h-5 ${isFavorite(String(artist.id)) ? "fill-white" : ""}`}
                  />
                </button>
              </div>
            </div>
                ...
                >
                  <div className="flex justify-between items-center">
                    ...
                    <div className="text-right">
                      ...
                      {service.popular && (
                        <span className="text-[10px] font-medium text-rose-500 bg-rose-50 dark:bg-rose-950/50 px-2 py-0.5 rounded-full">
                          Popular
                        </span>
                      )}
                    </div>
          ...
          <div>
            ...
            <div className="space-y-4">
              {reviews.map((review: any) => (
                <div
                  key={review.id}
                  className="p-4 bg-white dark:bg-neutral-900 rounded-xl border border-gray-100 dark:border-neutral-800"
                >
          ...
          <div className="p-6 bg-white dark:bg-neutral-900 rounded-2xl border border-gray-100 dark:border-neutral-800 sticky top-24">
            ...
            {!user ? (
              ...
              <div className="space-y-3">
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  Selected service:{" "}
                  {services.find((s) => String(s.id) === selectedService)?.name}
                </p>
              ...
              <div className="space-y-3">
                {similar.map((s: any) => (
                  <Link
                    key={s.id}
                    to={`/artists/${s.id}`}
                    className="flex items-center gap-3 p-3 bg-white dark:bg-neutral-900 rounded-xl border border-gray-100 dark:border-neutral-800 hover:border-rose-200 dark:hover:border-rose-800 transition-all"
                  >
                    <ImageWithFallback
                      src={s.image}
                      alt={s.name}
