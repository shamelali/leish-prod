...
import {
  Users, Building2, DollarSign, BarChart3, TrendingUp, Calendar, Star, Shield,
  Palette, Store, UserCheck, BookOpen, CreditCard, RefreshCw, CheckCircle, XCircle,
...
interface AdminStats {
  totalUsers: number; totalArtists: number; totalStudios: number;
  totalBookings: number; revenue: number; avgRating: number;
  pendingPayouts: number; newUsersThisMonth: number;
}

interface Artist { id: string; name: string; slug: string; email: string; phone: string; location: string; rating: string; reviewCount: number; verified: boolean; available: boolean; createdAt: string; }
interface Studio { id: string; name: string; slug: string; email: string; phone: string; location: string; rating: string; createdAt: string; }
...
export default function DashboardAdmin() {
  ...
  const toggleVerify = async (artistId: string, verified: boolean) => {
    ...
    await fetch('/api/admin?action=toggle-verify', {
      method: 'POST',
  ...
  const deleteArtist = async (artistId: string) => {
    ...
    setActionLoading(artistId);
    await fetch('/api/admin?action=delete-artist', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      ...
      {tab !== 'overview' && (
        <div className="relative mb-4">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 rounded-xl border border-gray-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 text-sm text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-rose-400"
          />
        </div>
      )}
              ...
              {recentActivity.map((item, i) => (
                <div key={i} className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-neutral-800 rounded-xl">
                  <div className={`p-1.5 rounded-lg ${
                    ...
                    {item.type === 'artist' ? <Palette className="w-4 h-4 text-violet-500" /> :
                     item.type === 'payment' ? <DollarSign className="w-4 h-4 text-green-500" /> :
                     item.type === 'booking' ? <Calendar className="w-4 h-4 text-blue-500" /> :
                     item.type === 'user' ? <UserCheck className="w-4 h-4 text-amber-500" /> :
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-neutral-800">
              {loading ? (
                ...
                <tr key={artist.id} className="hover:bg-gray-50 dark:hover:bg-neutral-800">
                  ...
                  <td className="p-3">
                    <span className="text-amber-500">{artist.rating}</span>
                    <span className="text-xs text-gray-400 ml-1">({artist.reviewCount})</span>
                  </td>
                  <td className="p-3">
                    <button onClick={() => toggleVerify(artist.id, !artist.verified)}
                  ...
                  <td className="p-3">
                    <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${
                      artist.available ? 'bg-green-50 text-green-600 dark:bg-green-950/30 dark:text-green-400' : 'bg-red-50 text-red-600 dark:bg-red-950/30 dark:text-red-400'
                    }`}>
                  ...
                  <td className="p-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <a href={`/artists/${artist.id}`} target="_blank" className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-neutral-700 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
                        <ExternalLink className="w-4 h-4" />
                      </a>
                      <button onClick={() => deleteArtist(artist.id)} disabled={actionLoading === artist.id}
                ...
                <tr key={user.id} className="hover:bg-gray-50 dark:hover:bg-neutral-800">
                  ...
                  <td className="p-3">
                    <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                      ...
                      user.role === 'artist' ? 'bg-violet-50 text-violet-600 dark:bg-violet-950/30 dark:text-violet-400' :
                      user.role === 'studio' ? 'bg-blue-50 text-blue-600 dark:bg-blue-950/30 dark:text-blue-400' :
                ...
                <tr key={b.id} className="hover:bg-gray-50 dark:hover:bg-neutral-800">
                  ...
                  <td className="p-3">
                    <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                      ...
                      b.status === 'pending' ? 'bg-amber-50 text-amber-600 dark:bg-amber-950/30' :
                      ...
                      'bg-gray-100 text-gray-600 dark:bg-neutral-800'
                    }`}>{b.status}</span>
                  </td>
                  <td className="p-3">
                    <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                      b.paymentStatus === 'paid' || b.paymentStatus === 'released' ? 'bg-green-50 text-green-600 dark:bg-green-950/30' :
                      b.paymentStatus === 'held' ? 'bg-blue-50 text-blue-600 dark:bg-blue-950/30' :
                      b.paymentStatus === 'pending' ? 'bg-amber-50 text-amber-600 dark:bg-amber-950/30' :
                      'bg-gray-100 text-gray-600 dark:bg-neutral-800'
