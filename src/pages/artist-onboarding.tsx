...
import { Check, ChevronDown, ChevronUp, Camera, User, Package, Calendar, CreditCard, MessageCircle, ClipboardList, LifeBuoy, ArrowLeft, LayoutDashboard } from 'lucide-react';
    ...
    items: [
      {
        ...
        content: 'In the luxury segment, your portfolio is your most valuable asset.',
        details: [
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        content: 'Our platform thrives on real-time bookings. Reliability is the hallmark of luxury.',
        details: [
    ...
    items: [
      {
        ...
        content: 'Leish! handles secure deposits to protect your time and prevent no-shows.',
        details: [
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-rose-50/50 to-white dark:from-neutral-950 dark:to-neutral-900">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        {/* Back + Header */}
        <div className="mb-8">
          <Link to="/dashboard/artist" className="inline-flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 hover:text-rose-600 dark:hover:text-rose-400 transition-colors mb-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 dark:text-white">Artist Onboarding Guide</h1>
              <p className="text-gray-500 dark:text-gray-400 mt-1">Welcome to the Leish! Elite Network</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-gray-200 dark:border-neutral-800 p-6 sm:p-8 mb-8 shadow-sm">
          <p className="text-gray-600 dark:text-gray-300 leading-relaxed">
            Welcome to the <strong className="text-gray-900 dark:text-white">Leish! Elite Network</strong>. As a luxury beauty marketplace, our goal is to pair Malaysia's premier artists with high-end clients. This guide is designed to help you set up a world-class profile that captures attention and secures bookings.
          </p>
        </div>
        ...
        <div className="space-y-4 mb-8">
          {phases.map((phase, i) => {
            ...
            return (
              <div key={i} className="bg-white dark:bg-neutral-900 rounded-2xl border border-gray-200 dark:border-neutral-800 overflow-hidden shadow-sm transition-shadow hover:shadow-md">
                <button
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <span className="text-xs font-semibold text-rose-500 dark:text-rose-400 uppercase tracking-wider">{phase.subtitle}</span>
                      <h2 className="text-lg font-semibold text-gray-900 dark:text-white">{phase.title}</h2>
                    </div>
                  </div>
                  ...
                  <div className="px-5 sm:px-6 pb-6 space-y-6 border-t border-gray-100 dark:border-neutral-800 pt-5">
                    {phase.items.map((item, j) => (
                      <div key={j}>
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400 mb-3">{item.content}</p>
                        <ul className="space-y-2">
                          {item.details.map((detail, k) => (
                            <li key={k} className="flex items-start gap-2.5 text-sm text-gray-600 dark:text-gray-300">
            ...
            {checklist.map((item, i) => (
              <label
                ...
                className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all ${
                  checked[i] ? 'bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-900/50' : 'bg-gray-50 dark:bg-neutral-800/50 border border-gray-100 dark:border-neutral-800 hover:border-rose-200 dark:hover:border-rose-800'
                }`}
              >
                <div
                  onClick={() => toggleCheck(i)}
                  className={`w-5 h-5 rounded-md border-2 flex items-center justify-center shrink-0 transition-all ${
                    checked[i] ? 'bg-emerald-500 border-emerald-500' : 'border-gray-300 dark:border-neutral-600'
                  }`}
                >
                  {checked[i] && <Check className="w-3.5 h-3.5 text-white" />}
                </div>
                <span className={`text-sm font-medium ${checked[i] ? 'text-emerald-700 dark:text-emerald-300 line-through' : 'text-gray-700 dark:text-gray-200'}`}>
                  {item.label}
                </span>
              </label>
            ))}
        ...
        <div className="bg-gradient-to-br from-rose-500 to-pink-600 rounded-2xl p-6 sm:p-8 text-white shadow-lg shadow-rose-200/50 dark:shadow-rose-900/30">
          ...
          <p className="text-white/80 text-sm mb-4">Need help perfecting your profile?</p>
