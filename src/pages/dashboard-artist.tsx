...
import { BarChart3, Calendar, DollarSign, Star, TrendingUp, ChevronLeft, ChevronRight, Check, X, Banknote, Wallet, Clock } from 'lucide-react';
...
export default function DashboardArtist() {
  ...
  const [stats, setStats] = useState({ bookings: 0, revenue: 0, rating: 0, reviews: 0 });
      ...
      try {
        ...
        if (bookingsRes.ok) {
          ...
          setStats({
            ...
            revenue: allBookings.reduce((sum: number, b: any) => b.status === 'completed' ? sum + Number(b.price || 0) : sum, 0),
  ...
  const filtered = bookings.filter((b: any) => bookingsFilter === 'all' || b.status === bookingsFilter);
        ...
        {showBankForm && (
          <form onSubmit={handleRegisterBank} className="mb-6 p-4 bg-gray-50 dark:bg-neutral-800 rounded-xl space-y-3">
            <input
              ...
              className="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 text-sm focus:outline-none focus:ring-2 focus:ring-rose-400"
              required
            />
            <input
            ...
            <div className="flex gap-2">
              <button type="submit" className="px-4 py-2 text-sm font-medium rounded-xl bg-rose-500 text-white hover:bg-rose-600 transition-colors">
                Save
              </button>
              <button type="button" onClick={() => setShowBankForm(false)} className="px-4 py-2 text-sm font-medium rounded-xl border border-gray-200 dark:border-neutral-700 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-neutral-800 transition-colors">
                Cancel
              </button>
            </div>
          </form>
              ...
              {payouts.slice(0, 5).map((p) => (
                <div key={p.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-neutral-800 rounded-xl">
                  ...
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                    p.status === 'released' ? 'bg-green-50 text-green-600 dark:bg-green-950/30 dark:text-green-400' :
                    p.status === 'pending' ? 'bg-amber-50 text-amber-600 dark:bg-amber-950/30 dark:text-amber-400' :
