...
export default function Favorites() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/artists`)
      .then((r) => r.json())
      .then((data) => {
        const favArtists = data.artists.filter((a: any) => favorites.includes(String(a.id)));
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex items-center gap-3 mb-8">
        ...
        <span className="px-3 py-1 text-sm font-medium bg-rose-50 dark:bg-rose-950/50 text-rose-600 dark:text-rose-400 rounded-full">{favorites.length}</span>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-gray-400 mb-6">Start browsing and save artists you love!</p>
          <Link to="/artists" className="inline-flex px-6 py-3 bg-gradient-to-r from-rose-500 to-pink-600 text-white font-semibold rounded-xl hover:from-rose-600 hover:to-pink-700 transition-all">
            Browse Artists
          </Link>
        </div>
      ) : (
        <>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {artists.map((artist: any) => (
              <div key={artist.id} className="group bg-white dark:bg-neutral-900 rounded-2xl overflow-hidden border border-gray-100 dark:border-neutral-800 hover:border-rose-200 dark:hover:border-rose-800 shadow-sm transition-all">
