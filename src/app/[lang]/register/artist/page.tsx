...
import { Sparkles, Mail, Lock, Eye, EyeOff, ArrowRight, User, Phone, Camera, Upload, Star, MapPin, Clock, Check, ChevronRight, Building2 } from "lucide-react";
import Navbar from "@/components/Navbar";
import Footer from "@/components/Footer";
import { categories } from "@/data/artists";

export default function RegisterArtistPage({ params }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(false);
    router.push(`/${locale}/register/artist/success`);
  };
      ...
      <div className="flex items-center justify-center min-h-[calc(100vh-4rem)] px-4 py-20">
        <div className="w-full max-w-2xl">
          ...
          <div className="grid grid-cols-3 gap-2 mb-6 animate-fade-in-up delay-75">
            <Link href={`/${locale}/register`} className="flex flex-col items-center gap-1.5 px-2 py-3 bg-white dark:bg-neutral-800 border-2 border-gray-100 dark:border-neutral-700 hover:border-rose-300 dark:hover:border-rose-700 rounded-xl text-center transition-all">
            ...
            <div className="flex flex-col items-center gap-1.5 px-2 py-3 bg-amber-50 dark:bg-amber-950/30 border-2 border-amber-500 rounded-xl text-center">
              <Camera className="w-5 h-5 text-amber-600 dark:text-amber-400" />
              <p className="text-xs font-semibold text-gray-900 dark:text-white">Artist</p>
            </div>
          ...
          <div className="flex items-center justify-center gap-4 mt-6 text-sm">
            <Link href={`/${locale}/register`} className="text-gray-500 dark:text-gray-400 hover:text-rose-600 dark:hover:text-rose-400 font-medium transition-colors">Client Sign Up</Link>
            ...
            <span className="text-gray-300 dark:text-neutral-700">|</span>
            <Link href={`/${locale}/login`} className="text-gray-500 dark:text-gray-400 hover:text-rose-600 dark:hover:text-rose-400 font-medium transition-colors">Already have an account?</Link>
          </div>
        </div>
      </div>
