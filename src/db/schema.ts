...
export const artistCategories = pgTable('artist_categories', {
  ...
  primaryKey({ columns: [table.artistId, table.categoryId] }),
]);
...
export const studioCategories = pgTable('studio_categories', {
  ...
  primaryKey({ columns: [table.studioId, table.categoryId] }),
]);
...
export const payouts = pgTable('payouts', {
  ...
  amount: integer('amount').notNull(),
  status: varchar('status', { length: 50 }).default('pending'),
  bankAccountId: integer('bank_account_id').references(() => muaBankAccounts.id, { onDelete: 'set null' }),
