...
export function checkLoginBlocked(ip: string, email: string): BlockResult {
  ...
  if (
    ipBucket &&
    Date.now() < ipBucket.resetAt &&
    ipBucket.count > IP_MAX_ATTEMPTS
  ) {
  ...
  if (
    emailBucket &&
    Date.now() < emailBucket.resetAt &&
    emailBucket.count > EMAIL_MAX_ATTEMPTS
  ) {
  ...
  if (
    globalBucket &&
    Date.now() < globalBucket.resetAt &&
    globalBucket.count > GLOBAL_MAX_ATTEMPTS
  ) {
