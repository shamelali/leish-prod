      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {categories.slice(0, 4).map((cat) => (
                <li key={cat.id}><Link href={`/artists?category=${cat.id}`} className="text-sm text-gray-400 hover:text-rose-400 transition-colors flex items-center gap-2"><span className="text-xs">{cat.icon}</span>{(t.categories as Record<string, any>)[cat.id] ?? cat.name}</Link></li>
              ))}
            </ul>
          </div>
          <div>
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              <li><Link href="/#how-it-works" className="text-sm text-gray-400 hover:text-rose-400 transition-colors">{t.howItWorks.title}</Link></li>
              <li><Link href="/artists" className="text-sm text-gray-400 hover:text-rose-400 transition-colors">{t.footer.artists}</Link></li>
              <li><Link href="/studios" className="text-sm text-gray-400 hover:text-rose-400 transition-colors">{t.footer.studios}</Link></li>
