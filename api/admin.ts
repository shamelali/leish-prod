  ...
  try {
    switch (action) {
      ...
      default: {
        ...
        const artistCountResult = await pool.query(
          `SELECT COUNT(*)::int as count FROM artists`,
        );
        const studioCountResult = await pool.query(
          `SELECT COUNT(*)::int as count FROM studios`,
        );
        ...
        const ratingResult = await pool.query(
          `SELECT COALESCE(ROUND(AVG(rating)::numeric, 2), 0)::float as avg FROM artists WHERE rating > 0`,
        );
        const payoutResult = await pool.query(
          `SELECT COALESCE(SUM(amount), 0)::int as total FROM payments WHERE status = 'held'`,
        );
