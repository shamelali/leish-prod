...
import { bookingExpiredTemplate, bookingAutoCanceledTemplate, bookingReminderTemplate } from '../../src/lib/email/templates';
  ...
  try {
    switch (action) {
      ...
      case 'release-escrow': {
        ...
        await pool.end();
        return new Response(JSON.stringify({ released, total: heldPaymentsResult.rows.length }), {
          status: 200,
          headers: { 'Content-Type': 'application/json' },
        });
      }
      ...
      case 'reminders': {
        ...
        await pool.end();
        return new Response(JSON.stringify({ ok: true, sent, failed, total: bookingsResult.rows.length }), {
          status: 200,
          headers: { 'Content-Type': 'application/json' },
        });
      }
