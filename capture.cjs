...
(async () => {
  ...
  // Studio detail
  await page.goto('http://localhost:3456/studios', { waitUntil: 'networkidle', timeout: 15000 });
  const studioLink = await page.$('a[href^="/studios/"]');
  if (studioLink) {
    const href = await studioLink.getAttribute('href');
    await page.goto('http://localhost:3456' + href, { waitUntil: 'networkidle', timeout: 15000 });
    await page.screenshot({ path: 'screenshots/studio-detail.png', fullPage: false });
    console.log('✓ studio-detail');
  }

  // Blog detail
  await page.goto('http://localhost:3456/blog', { waitUntil: 'networkidle', timeout: 15000 });
  const blogLink = await page.$('a[href^="/blog/"]');
  if (blogLink) {
    const href = await blogLink.getAttribute('href');
    await page.goto('http://localhost:3456' + href, { waitUntil: 'networkidle', timeout: 15000 });
    await page.screenshot({ path: 'screenshots/blog-detail.png', fullPage: false });
